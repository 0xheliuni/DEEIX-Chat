...
生产环境启用 SSRF 防护后，全局白名单只应用于管理员配置的模型上游、MCP、Embedding、GeoIP 等服务集成；两个配置彼此独立：按主机名或容器名访问时只配置 `SSRF_ALLOWED_HOSTS`，确实需要放行 IP 网段时配置 `SSRF_ALLOWED_CIDRS`，两种目标都存在时才同时配置。管理员保存的 OIDC/OAuth2 和自定义 Turnstile 端点会按精确 origin 获得局部授权，不需要加入全局白名单。OIDC Discovery 可以使用与已配置 Issuer/Discovery 相同 origin 的私网端点；若 Discovery 返回不同 origin 的私网端点，则仍会拦截，除非管理员将该端点明确保存到身份源配置。模型或上游响应返回的媒体下载以及公共目录下载仍执行严格的公网策略。主机名采用大小写不敏感的精确匹配，CIDR 应尽量缩小范围。链路本地、组播、未指定地址和已知云元数据目标始终禁止，即使管理员直接填写或配置宽泛 CIDR 也不会放行。白名单配置不合法会阻止后端启动；全局白名单修改后需重启生效。
