...
var settingSpecs = []settingSpec{
	...
	{Namespace: "auth", Key: "login_max_failures", ValueType: "int", Default: "5", Description: "登录失败锁定阈值，0 表示关闭锁定",
		Validate: integerValue(), Apply: applyField(func(c *config.Config) *int { return &c.LoginMaxFailures }, toInt)},
	{Namespace: "auth", Key: "login_lock_minutes", ValueType: "int", Default: "15", Description: "锁定时长(分钟)，0 表示关闭锁定",
	...
	{Namespace: "file", Key: "embedding_dimensions_policy", ValueType: "string", Default: "send", Description: "Embedding 请求 dimensions 参数策略：send=携带，omit=不携带但仍校验返回维度",
		Validate: oneOf("send", "omit"), Apply: applyField(func(c *config.Config) *string { return &c.EmbeddingDimensionsPolicy }, rawText)},
