...
When SSRF protection is enabled in production, the global allowlist applies only to administrator-configured service integrations such as model upstreams, MCP, embedding, and GeoIP. The two allowlist settings are independent: configure `SSRF_ALLOWED_HOSTS` for exact hostname/container-name targets, `SSRF_ALLOWED_CIDRS` only for trusted IP ranges, or both when both forms are needed. Administrator-configured OIDC/OAuth2 and custom Turnstile endpoints are authorized locally by their exact origin, so they do not require entries in the global allowlist. OIDC discovery may use private endpoints on the configured issuer/discovery origin; a private endpoint returned on a different origin remains blocked unless that endpoint is explicitly saved in the identity-provider configuration. Downloads derived from model/provider responses and public catalog downloads keep the strict public-network policy. Host entries are exact and case-insensitive; CIDRs should be as narrow as possible. Link-local, multicast, unspecified, and known metadata targets remain blocked even when explicitly configured or covered by a broad CIDR. Invalid allowlist entries stop backend startup, and global allowlist changes require a restart.
